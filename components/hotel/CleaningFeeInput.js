...
export default function CleaningFeeInput({ value, onChange }) {
  ...
  const [canOpenOnFocus, setCanOpenOnFocus] = useState(true); // 控制定点开关
  ...
  const handleFocusOrClick = () => {
    if (v.mode !== "paid") return; // 免费时不需要展开列表
    if (canOpenOnFocus) {
      setIsOpen(true);
    }
  };
  ...
  const handleSelectPrice = (amount) => {
    ...
    // 选中价格后：收起列表，并暂时禁止再自动展开
    update({ value: val });
    setIsOpen(false);
    setCanOpenOnFocus(false);
  };

  // 点击外面时，收起下拉，并把 canOpenOnFocus 重置为 true
  useEffect(() => {
    const handleClickOutside = (e) => {
      if (wrapperRef.current && !wrapperRef.current.contains(e.target)) {
        setIsOpen(false);
        setCanOpenOnFocus(true); // 下次再点输入框才会重新展开
      }
    };
