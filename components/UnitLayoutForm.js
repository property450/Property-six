...
export default function UnitLayoutForm({ index, data, onChange }) {
  ...
  // ⭐ 关键：这里的 config 要和 subsale 用的一模一样的结构
  const config = {
    ...
    orientation: layout.facing || "", // 朝向（和 subsale 一样，用 facing）
    transit: layout.transit || null,  // 虽然不会生成上传框，但保留字段没问题
  };
  ...
  return (
    <div className="border rounded-lg p-4 shadow-sm bg-white">
      ...
      <RoomCountSelector
        ...
        onChange={(patch) => {
          // patch 里一般是 { bedrooms, bathrooms, ... }，和 subsale 一样直接 merge
          updateLayout(patch);
        }}
      />
      ...
      {/* ⭐ 根据 config 生成卧室/浴室/厨房/客厅/车位/家私/设施/额外空间/朝向 的照片上传框 */}
      <div className="mb-3">
        <label className="block mb-1 font-medium">上传此 Layout 的照片</label>
        <ImageUpload
          config={config}
          images={layout.photos || {}} // 和 subsale 一样用「对象」结构
